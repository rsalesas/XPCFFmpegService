...
//  Builds a `char **`-style pointer array from a [String], for calling
//  C main(argc, argv)-style entry points like ffmpeg()/ffprobe().
//
...
struct CStringArray {
    ...
    subscript(index: Data.Index) -> UnsafeMutablePointer<CChar>? {
        get {
    ...
    subscript(index: Data.Index) -> String? {
        get {
            precondition(index >= 0 && index < count, "Index out of range")
